...
static func can_purchase(entry: Dictionary, tier: int, progression_flags: Dictionary) -> bool:
	var min_t := int(entry.get("min_wealth_tier", 0))
	var req_flag := str(entry.get("requires_flag", ""))
	var tier_ok := tier >= min_t
	var flag_ok := true
	if req_flag != "":
...
static func visible_in_shop(entry: Dictionary, tier: int, progression_flags: Dictionary) -> bool:
